...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **The dataframe now only includes the information for Kamala Harris and Tim Walz for each Maryland jurisdiction. The column name is pretty long however. **
...
**Task** Run the following code to extract the table we want, clean up the names and remove the total row. Look at the rename() function - what do you think the numbers 2 through 6 are referring to? **The numbers 2 through 6 in the rename() function refer to the column positions in the original data frame. They represent the second, third, fourth, fifth, and sixth columns, which are being renamed to harris, trump, oliver, stein, and kennedy.**
...
**Task** Run the following code in the gray-colored codeblock below to find the <li> tags. What does the output suggest about how many <ul> tags there are in the HTML source? **It suggests that there are at least a few <ul> tags in the HTML source. It doesn't directly tell us how many, but the number of <ul> tags is likely fewer than 55. **
...
**Task** Run the following code in the gray-colored codeblock below to find the text inside <li> tags. Describe the output: **It shows the text from each <li> tag. It consists of the labels from the list items, such as "Representatives," "Leadership," "Committees," etc.**
...
### Task 11: Turn the contents of the <li> and <a> tags into a dataframe **Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: ** The output is a tibble with two columns: name and url. "name" has the the name of a committee or section."url" holds the links for each committee or section.**
