...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **There are ten different columns: Precinct ID, Precinct Name, Congress, Legislative district, council district, polling ID, shape length, shape area, and shape. The last three are about the area of a precinct. **
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **The output from the space below presents the data frame in a spreadsheet-like format. It contains only precinct-level data. **
...
**Task** Run the following code. Describe the output in the space below. **It produced a geographic map representing the election precincts in PG County. The color is gray, with each of the precincts being divided. .**
...
**Task** Run the following code. Describe the output in the space below. **The code loads the tigris package, which provides access to U.S. geographic and boundary data. By setting options(tigris_use_cache = TRUE), users can speed up data retrieval by saving previously accessed information for future use.**
...
**Task** Run the following code. Describe the output in the space below. **The code retrieves a dataset containing 3,235 rows and 18 columns of U.S. county-level geographic data for 2022. There are details like state and county FIPS codes, county names, land and water areas, and geographic boundaries. **
...
**Task** Run the following code. Describe the output in the space below. **The dataset only contains counties in Maryland now, with the STATEFP number 24, which represents Maryland. **
...
**Task** Run the following code. Describe the output in the space below. **There is another chloropleth map, this time showing the state of Maryland. It has all the counties in Maryland shown, with a minimal theme that is mostly gray.**
...
**Task** Run the following code. Describe the output in the space below. **It is a chloropleth map of Prince George's county. It shows all the districts. It has a minimal theme still and is gray. **
...
**Task** Run the following code. Describe the output in the space below. **It shows the vote count for each precinct in Prince George's County. The columns are: precinct, total polls, total early, total absentee, total provisional, and total eligible.**
...
**Task** Run the following code. Describe the output in the space below. **The dataset has combined the pg_turnout data with the pg_precincts data. It now includes congressional, school, legislative, council, polling ids and thegeometry information with the voting information. **
...
**Task** Run the following code. Describe the output in the space below. **The chloropleth map now is shaded in each precinct by the amount of total eligible voters. The darker the blue, the less eligible voters there are.**
...
**Task** Run the following code. Describe the output in the space below. **It shows the amount of eligible voters in each county. This is divided by the party that the voter is registered as.**
...
**Task** Run the following code. Describe the output in the space below. **It now includes the estimated population for each county in Maryland. There is no margin of error listed. **
...
**Task** Run the following code. Describe the output in the space below. **It shows that the data contains a multipolygon geometry for each county. There is also GEOID, NAME, variable, estimate and geometry variables, which allow it to visualize the county boundaries and population data. **
...
**Task** Run the following code. Describe the output in the space below. **This shows the voting data for each Maryland county. It shows the total number of voters in each county, and then the amount of registered voters for each political party. **
...
**Task** Run the following code. Describe the output in the space below. **It is already named COUNTY and there is an error with the rename function as NAME does not exist. **
...
**Task** Run the following code. Describe the output in the space below. **It shows the geometric data of each county with the population totals. It also contains the voter info by party for each county. **
...
**Task** Run the following code. Describe the output in the space below. **It contains only the county name, population, and geometry of the county. **
...
**Task** Run the following code. Describe the output in the space below. **The dataset has now been mutated and contains a new column, "voters_per_10k". This shows the amount of voters per a 10k population in each county. **
...
**Task** Run the following code. Describe the output in the space below. **There is a chloropleth map showing the amount of voters per a 10k population in Maryland's counties. The lighter the blue the more voters there are in that respective county.**
...
**Task** Run the following code. Describe the output in the space below. **It now isn't just blue as the only color. It is now blue, yellow, red, orange, and purples. This makes it easier to distinguish the county lines. **
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **The colors that are used are still the same. Now, the map and county lines are even more refined. Near D.C. there is actually one of the fewest amounts of voters, surprisingly. It is also surprising that Worcestor county, at the bottom tip of Maryland, has one of the highest rates. **
...
**Task** Run the following code. Describe the output in the space below. **It now contains the amount of unaffiliated voters per a 10k population for each of the Maryland counties. **
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see, especially on the ends of the scale? **It is interesting how Howard, Anne Arundel and Fredrick county all have higher amounts than Montgomery and PG county, considering that those are the two most populated states and the three border those two. **
