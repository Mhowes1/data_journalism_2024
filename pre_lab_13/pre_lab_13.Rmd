...
```{r}
create_credentials(GROQ_API_KEY = "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")
```
...
**I feel as if next nine names after the first, "Lemmy", are just as bad. Other than Lemmy, I don't see a correlation to lemurs other than the fact that those are pet names. 
...
**Llama got it all right and did not miss on any details. **
...
**Again, Llama got it right. In the case where there was missing data, I think a line could be added where the field would be marked as N/A, rather than the AI generating a response that could be wrong.  **
...
**I received error messages saying that R had trouble finding the pipe operator. **
...
**The  response was mostly correct, but it made assumptions about multi-party registration. In most U.S. voter systems, a voter can only register to one party. This analysis may not apply then. **
...
**"I can't extract or describe any information about images or visual content. However, I can provide general suggestions on how to describe images if that would be helpful." **
...
**Ollama would not open despite me installing it mutliple times. ** 
