...
**https://www.datawrapper.de/_/evK9q/** Copy the url of the graphic you published here. It will begin with <https://datawrapper.dwcdn.net/>. Don't copy the browser's current url.
...
md_gov_county <- md_gov_county |>
  ...
  mutate(psct_moore = Moore/total *100)
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **This version of the map shows only the Maryland counties. This is good, since there previously was the whole country, which was data-less and grey.** Describe the changes from the previous map
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "00.0%")}} **Now whenever I hover over a county, it will show the percentage and county name for that region.** Describe the changes from the previous map
...
**Task** Click the "Proceed" button until you get to the "Publish & Embed" tab, then click "Publish Now". Copy the published URL (like you did for the chart above) and paste it below. **https://www.datawrapper.de/_/fyzbL/** Put the URL of your map here.
