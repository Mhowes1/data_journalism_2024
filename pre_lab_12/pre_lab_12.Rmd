...
**Task** Run the following code and describe the dataframe it outputs. **It shows the URLS for 999 of Cardin's press releases. It also displays the date, title and text for each entry. **
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. ** The function removes duplicate values from the original list of words and keeps only the first occurrence of each unique element. **
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **There is a row for each unique word that is used in the press releases. There are 677,425 unique words used. **
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **It is not that interesting to be honest. It contains a lot of common words that would be expected from Cardin's press releases. **
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Yes. You can see where a lot of his priorities lie, with words like "health", "funding", "baltimore" and "communities" being present. **
...
**Task** Run the following code and describe the results. **It shows the top ten words that are used, and the percentage that each word is in the total sum of press releases. For example, "Baltimore" was the No. 1 word with 559 mentions and being in 63.4% of press releases in 2024. **
...
**Task** Run the following code and describe the results. **There were some intriguing results that appeared. Phrases like "health care", "covid 19", "Human rights" , "mental health" and "chesapeake bay" continue to hint at what Cardin's priorities are. **
...
**Task** Run the following code and describe the results. **A lot of similar results appear with the refined dataset. "Health care", "chesapeake bay" and "human rights" remain among the top results. **
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **The stem_words function reduces words to their root forms. The lemmatize_words function converts words to their base form, ensuring the result is a valid word --- for example, "driven" and "drives" both become "drive". For press release data, lemmatization is  more useful as it returns meaningful, standardized word forms.**
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **The unique_words_2024 dataset keeps the orginal form of the word. The unique_words_2024_lemma dataset has just the base form of a word --- which will narrow the set. The lemmatized set is more useful. **
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **It seems that any word that could have a negative conntation is labeled as negative, but I believe that it could be wrong for a few and is hard to determine without seeing the full text. For example, the word cancer is labeled as negative, but I also know "Bipartisan Legislation to Expand Genetic Cancer Testing" was one of the press releases, which is not negative. I don't believe this is a useful function. **
