...
```{r echo=FALSE}
#census_api_key("b0577e5bf0e44b2520dc678b6bcd133f169c026f", install=TRUE)
```
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Nevada had the largest percentage growth with 35.14%**
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Howard County has the highest median household income wth $140,971. **
...
Is the difference between Calvert and Montgomery significant? **It is not too significant as the bars of the two county on the graph overlap, meaning that the difference between the two counties is within the margin of error. **

Is the difference between Howard and everyone else significant? **It is significant between Howard County and everyone else. Not a single county overlaps with Howard, meaning that the difference is outside the margin of error for all the counties and Howard. **
...
**None of the top 20 zip codes with the largest amounts have an estimated median income less than $50,000.**
...
**Task** Arrange md_zips_with_acs by estimate showing the highest median household income first. What questions do you have about these top zip codes? **I'd like to learn more about zip code 21071. After the top zip code, $21071 had the largest margin of error in the top 20 zip codes. It's max is $309940 while it's min is $95372, the lowest in the top 20. I'd like to know why it seems the wealth in the zip code is very disproportionate between the top and bottom. **
