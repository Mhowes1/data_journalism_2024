...
**There are 131395 observations and 24 columns**
...
In the space below, write the data type for the "last_name" column and the "amount" columns (it's in light gray, offset by \< \>). Also offer your best guess as to what the "last_name" column and the "amount" column represent. **Last_name is <chr> and ammount is <dbl>. <chr> probably is character and <dbl> could be double **
...
**These results group the number of contributions by date. The results from 'head()' just represent random contributions. **
...
**The answer represents the days that saw the most contributions. **
...
**This shows the amount of times a certain amount of donated to a committee. For example, it shows that the Trump National Committee was donated one dollar 3,452 times. **
...
**This now shows the total ammount that was donated to a committee, as well the amount of contriutions.**
...
**It reveals the mean and the median of each different committee from its contributions. Hogan for Maryland is shown to have recieved the highest mean contribution.  **
...
Compared to the previous codeblock, what has changed and how did that affect the result? How is the data sorted? **This now shows the contribution data in revelance to the city the contributions are being made in. It shows the mean, median, min, and max amounts for each city, as well as the total amounts. **
