...
**Task** Load the five CSV files. Do they have the same column structure? **Yes, other than the "UNA" and "OTH" column. Those are swapped on a few of the data files. **
...
**Task** Load the Maryland 2020 county population data **This is the data loaded for Maryland 2020 county population data.**
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **There are 24 rows now, there should be 24 rows.**
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **It is saying that "left_join" is being used with the "County" column to combine the two data frames.**
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Prince George's County has both the highest 18+ Black population as well as the percentage of Democrats in the county.**
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **It is a decimal character type, able to hold numbers. **
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **It is now a character type of datatype. **
