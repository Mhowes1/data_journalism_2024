...
**Task** Check for problems that occurred when loading the Texas precinct results by running the following code. How many problems were there, as shown by the number of rows in the output table showing errors? What do you think the problem is that R is describing? Answer below. **It appears that there are 1,640 rows of problems. It seems the issue is that there are numeraric values in what is supposed to be a character column.**
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **There are no more parsing issues**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **The "district" and "mail" columns are both <dbl> while the "precinct" field is a chracter column. The <db> stands for double precision. It is a number that will have decimal points.**
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **There were 402,345 results when mail was NA. This implies that there was either missing data, where the precincts might not have reported mail-in votes from that cycle, or there were precincts that did not have mail-in voting as part of the election cycle. **:
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **Five that are request from voter are interesting, because that would signal that the individual asked to have themselves removed from the registry, which could show interesting reasons. **
...
**There are 14 rows in the demonstration data set.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **The "amount" field is a character data.**
...
**The amount column is a <chr> column, which does not work. There is also inconsistent zip-codes, with some five-digit and some nine-digit. There also misspellings and some city names with all-caps, other with no caps.  **
...
**It removed the leading digit to make it a valid name. Tge clean_names() function can be used to make column names more readable and consistent with the rest of the data. **
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **It says that there is an error in summarise with a invalid 'type' of character. I believe the issue is that it could not perform the calculation due to there being characters instead of numbers. **
...
**Task** Run the following codeblock to attempt to change the datatype for the amount field to a number. What is the new data type (three letter code) for amount? Answer below. **It is now <dbl>.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **There are two duplicates under Derrick Hamilton. **
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. Add a description of what this code does to your reference notebook. **It created 13 rows. The distinct () function removed duplicate rows.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **It standerized the data field with all five-digit ZIP codes. **
...
The function standerized the data field, which now has the first letter capitalized for all cities. 
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **There is one mispelling.**
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **There are no more misspellings. The code detected possible misspellings and corrected it was the right spelling of Conowingo. **
